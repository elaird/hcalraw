...
def opts():
    ...
    parser.add_option("--min", dest="min", default=None,
                      metavar="N",
                      help="specify minimum run number")

    parser.add_option("--max", dest="max", default=None,
                      metavar="N",
                      help="specify maximum run number")

    parser.add_option("--run", dest="run", default=None,
                      metavar="N",
                      help="specify one run number")

    parser.add_option("--only-utca", dest="onlyutca", default=False,
                      action="store_true",
                      help="ignore uTCA file if present")

    parser.add_option("--only-cms", dest="onlycms", default=False,
                      action="store_true",
                      help="ignore CMS file if present")

    parser.add_option("--filter-evn", dest="filterevn", default=False,
                      action="store_true",
                      help="consider only events with EvN&0x1fff == 0")

    parser.add_option("--only-summary", dest="onlysummary", default=False,
                      action="store_true",
                      help="do not loop;" +
                      " simply make summary plots from existing ROOT files")
...
def rootFiles(directory="", mode=""):
    ...
    for item in sorted(os.listdir(directory2)):
        if mode == "usc":
            ...
            if len(fields) < 3:
                continue
            if fields[0] != "USC" or fields[1] != "uTCA":
                continue
...
def eosFile(run):
    ...
    subDir = "store/hidata/HIRun2013/HcalNZS/RAW/v1/000"
    dir = "%s/%s/%03d/%03d/00000/" % (castor, subDir, run/1000, run % 1000)
