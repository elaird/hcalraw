...
int main(int argc, char* argv[]) {
  gROOT->ProcessLine("#include <vector>; #pragma link C++ class vector<uint64_t>+;"); 
  ...
  TFile a(Form("badcoffee%s.root", argv[1]), "RECREATE");
  TTree tree("badcoffeetree","");
  
  const int nHeader = 2;  // MAGIC, nWords64

  int iWordInBlock = 0;
  std::vector<uint64_t> vec(1 << 13);
  vec.clear();
